<?php
/**
 * Envo Royal Customizer Notification System.
 *
 * @package Envo Royal
 */
...
/**
 * Envo Royal Customizer Notify Class
 */
#[AllowDynamicProperties]
class Envo_Royal_Customizer_Notice {
	...
	/**
	 * The single instance of Envo_Royal_Customizer_Notice
	 *
	 * @var Envo_Royal_Customizer_Notice $instance The Envo_Royal_Customizer_Notice instance.
	 */
	...
	/**
	 * The Main Envo_Royal_Customizer_Notice instance.
	 *
	 * We make sure that only one instance of Envo_Royal_Customizer_Notice exists in the memory at one time.
	 *
	 * @param array $config The configuration array.
	 */
	public static function init( $config ) {
		if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Envo_Royal_Customizer_Notice ) ) {
			self::$instance = new Envo_Royal_Customizer_Notice;
	...
	public function setup_config() {
		...
		global $envo_royal_recommended_plugins;
		...
		$envo_royal_recommended_plugins = array();
		
		if ( isset( $this->recommended_plugins ) ) {
			$envo_royal_recommended_plugins = $this->recommended_plugins;
		}
	...
	public function scripts_for_customizer() {
		...
		wp_enqueue_style( 'envo-royal-customizer-notice', get_template_directory_uri() . '/extra/customizer/notice/css/customizer-notice.css', array() );
		...
		wp_enqueue_script( 'envo-royal-customizer-notice', get_template_directory_uri() . '/extra/customizer/notice/js/customizer-notice.js', array( 'customize-controls' ) );
		wp_localize_script(
			'envo-royal-customizer-notice', 'customizer_notice_data', array(
				'ajaxurl'                  => admin_url( 'admin-ajax.php' ),
				'template_directory'       => get_template_directory_uri(),
				'base_path'                => admin_url(),
				'activating_string'        => esc_html__( 'Activating', 'envo-royal' ),
			)
		);
	...
	public function customize_register_notice( $wp_customize ) {
		...
		/**
		 * Include the Envo_Royal_Customizer_Notice_Section class.
		 */
		require_once get_template_directory() . '/extra/customizer/notice/class-customizer-notice-section.php'; // phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound

		$wp_customize->register_section_type( 'Envo_Royal_Customizer_Notice_Section' );

		$wp_customize->add_section(
			new Envo_Royal_Customizer_Notice_Section(
				$wp_customize,
	...
	public function dismiss_recommended_plugins_callback() {
		...
		if ( ! empty( $action_id ) ) {
			...
			$show_recommended_plugins = get_option( 'envo_royal_show_recommended_plugins' );
			if ( isset( $_GET['todo'] ) ) {
				switch ( $_GET['todo'] ) {
					case 'add':
			...
			update_option( 'envo_royal_show_recommended_plugins', $show_recommended_plugins );
		}
