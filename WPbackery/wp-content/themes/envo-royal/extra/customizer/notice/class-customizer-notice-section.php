<?php
/**
 * Envo Royal Customizer Notification Section Class.
 *
 * @package Envo Royal
 */

/**
 * Envo_Royal_Customizer_Notice_Section class
 */
class Envo_Royal_Customizer_Notice_Section extends WP_Customize_Section {
	/**
	...
	public function json() {
		...
		foreach ( $envo_royal_recommended_plugins as $slug => $plugin_opt ) {
			...
			$envo_royal_recommended_plugin['url']    = $this->create_action_link( $active['needs'], $slug );
			if ( $active['needs'] !== 'install' && $active['status'] ) {
				$envo_royal_recommended_plugin['class'] = 'active';
			} else {
				$envo_royal_recommended_plugin['class'] = '';
			}

			switch ( $active['needs'] ) {
				case 'install':
					$envo_royal_recommended_plugin['button_class'] = 'install-now button';
					$envo_royal_recommended_plugin['button_label'] = $install_button_label;
					break;
				case 'activate':
					$envo_royal_recommended_plugin['button_class'] = 'activate-now button button-primary';
					$envo_royal_recommended_plugin['button_label'] = $activate_button_label;
					break;
				case 'deactivate':
					$envo_royal_recommended_plugin['button_class'] = 'deactivate-now button';
					$envo_royal_recommended_plugin['button_label'] = $deactivate_button_label;
					break;
			}
			$info   = $this->call_plugin_api( $slug );
			$envo_royal_recommended_plugin['id'] = $slug;
			$envo_royal_recommended_plugin['plugin_slug'] = $slug;

			if ( ! empty( $plugin_opt['description'] ) ) {
				$envo_royal_recommended_plugin['description'] = $plugin_opt['description'];
			} else {
				$envo_royal_recommended_plugin['description'] = $info->short_description;
			}

			$envo_royal_recommended_plugin['title'] = $info->name;

			$customize_plugins[] = $envo_royal_recommended_plugin;
				...
				<div class="recomended-actions_container" id="plugin-filter">
					<# if( data.recommended_plugins.length > 0 ){ #>
						<# for (action in data.recommended_plugins) { #>
							<div class="envo-royal-recommeded-actions-container envo-royal-recommended-plugins" data-index="{{ data.recommended_plugins[action].index }}">
								<# if( !data.recommended_plugins[action].check ){ #>
									<div class="envo-royal-recommeded-actions">
